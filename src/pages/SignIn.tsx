...
const SignIn = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    navigate(roles.includes("recipient") ? "/portfolio" : "/dashboard");
  };
  ...
  return (
    <AuthShell
      ...
      subtitle={forgotMode ? "Enter your email to receive a reset link" : "Sign in to your AfriCertify account"}
      footer={
        <>
          Don't have an account?{" "}
          <Link to="/signup" className="text-secondary hover:text-secondary/80 font-medium transition-colors">
            Sign up for free
          </Link>
        </>
      }
    >
      <form onSubmit={forgotMode ? handleForgotPassword : handleSignIn} className="space-y-5">
          ...
          <>
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="px-3 bg-[hsl(var(--surface-2)/0.9)] text-muted-foreground tracking-wider">OR CONTINUE WITH</span>
              </div>
            </div>
